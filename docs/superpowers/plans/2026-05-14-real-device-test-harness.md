...
**Architecture:** 纯 shell。**驱动层**（`run.sh` + `lib/harness.sh`）跑在开发机（darwin/bash），通过 `ssh` 操控路由器；**探针**（`lib/probe.sh`）是 busybox-ash 兼容脚本，经 `ssh ... sh -s` 推到路由器或 LAN 客户端执行。每个用例是 `cases/<ID>_<name>.sh`，source `config.sh` + `harness.sh`，以退出码 0/1/2 = PASS/FAIL/SKIP 收尾并打印 `RESULT` 行。`run.sh` 发现、过滤、汇总。纯逻辑函数（三态分类、最长连续窗口、用例选择匹配）用 TDD；SSH/路由器交互的胶水代码用 `bash -n` + `--dry-run` + 真机运行验证。
...
tests/realdevice/
  ...
  lib/
    probe.sh                 # 三态连通性探针（busybox-ash，路由器/客户端侧执行）
...
**分层职责：** `probe.sh` 只判连通性（OK/FAIL），不碰路由器状态；`harness.sh` 组合连通性 + iptables 真值得出三态，并提供所有 SSH/状态/故障注入/资源操作函数；`cases/*.sh` 只声明式调用 harness 函数；`run.sh` 只做编排。纯逻辑（`classify`/`max_contiguous`/`case_matches`）独立可测。
...
# probe.sh 用 curl 时只看 HTTP code；这里 stub curl 验证 OK/FAIL 分类。
set -u
...
assert_eq "$(PATH="$TMP:$PATH" sh "$HERE/probe.sh" 2)" "OK" "curl 204 → OK"

# stub curl → HTTP 000 (timeout/conn refused)
printf '#!/bin/sh\nprintf 000\n' > "$TMP/curl"
assert_eq "$(PATH="$TMP:$PATH" sh "$HERE/probe.sh" 2)" "FAIL" "curl 000 → FAIL"

# stub curl → HTTP 502 (reachable but bad)
printf '#!/bin/sh\nprintf 502\n' > "$TMP/curl"
assert_eq "$(PATH="$TMP:$PATH" sh "$HERE/probe.sh" 2)" "FAIL" "curl 502 → FAIL"
...
# 三态测试套件的连通性探针。busybox-ash 兼容；在路由器或 LAN 客户端上执行。
# 输出恰好一行：OK（能在超时内拿到 generate_204）或 FAIL（不能）。
# 用法：sh probe.sh [curl_timeout_seconds]   默认 5s
set -u
TIMEOUT="${1:-5}"
...
if command -v curl >/dev/null 2>&1; then
    ...
    if [ "$code" = "204" ]; then echo OK; else echo FAIL; fi
else
    # busybox wget 兜底：成功（含 204）退出 0。
    if wget -q -T "$TIMEOUT" -O /dev/null "$URL" 2>/dev/null; then echo OK; else echo FAIL; fi
fi
```
...
# classify <conn> <rules_rc>
assert_eq "$(classify OK 0)"   "PROXY"     "OK + rules present → PROXY"
assert_eq "$(classify OK 1)"   "DIRECT"    "OK + rules absent → DIRECT"
assert_eq "$(classify FAIL 0)" "BLACKHOLE" "FAIL + rules present → BLACKHOLE"
assert_eq "$(classify FAIL 1)" "WANDOWN"   "FAIL + rules absent → WANDOWN"
...
Expected: FAIL —— `harness.sh: No such file or directory`
...
# classify <conn:OK|FAIL> <rules_rc:0|1> → PROXY|DIRECT|BLACKHOLE|WANDOWN
# 三态判定核心。rules_rc=0 表示 iptables 'sing-box' 链存在。
classify() {
    case "$1" in
        OK)  if [ "$2" -eq 0 ]; then echo PROXY; else echo DIRECT; fi ;;
        *)   if [ "$2" -eq 0 ]; then echo BLACKHOLE; else echo WANDOWN; fi ;;
    esac
}
...
Run: `bash tests/realdevice/lib/harness_test.sh`
Expected: PASS —— `harness_test: 14 passed, 0 failed`
...
# probe_conn [timeout_s] → OK|FAIL : 在 LAN 客户端（若配置）否则路由器上跑 probe.sh
probe_conn() {
    local to="${1:-5}" tgt="$ROUTER_SSH"
    [ -n "${LAN_CLIENT_SSH:-}" ] && tgt="$LAN_CLIENT_SSH"
    ssh "${_RD_SSH_OPTS[@]}" "$tgt" sh -s -- "$to" < "$HARNESS_DIR/probe.sh"
}

# rules_present : rc 0 若 nat 表 'sing-box' 链存在（iptables 真值）
rules_present() { rsh "iptables -t nat -nL sing-box >/dev/null 2>&1"; }

# probe [timeout_s] → PROXY|DIRECT|BLACKHOLE|WANDOWN : 完整三态判定
probe() {
    local to="${1:-5}" conn rc
    conn="$(probe_conn "$to")"
...
# 复制为 config.sh 后按实机修改。config.sh 已被 .gitignore 忽略 —— 切勿提交凭证。
ROUTER_SSH="root@192.168.1.1"          # 路由器 ssh 目标（host 或 ssh_config 别名）
LAN_CLIENT_SSH=""                      # 可选：LAN 侧客户端 ssh 目标；
                                       # 留空 → 从路由器自身做连通性探针
...
Expected: probe_test 3 passed、harness_test 14 passed、dry-run 无用例输出，整体退出码 0
...
- 一台已 **正常运行** sing-router 的华硕路由器（koolshare 固件为主），SSH 可达。
- 开发机已装 `jq`、`ssh`、`bash`。
- 路由器侧具备 `curl` 或 `wget`、`iptables`、`ip`、`ipset`、`nslookup`（Entware/busybox 标配）。
...
- `lib/probe.sh` —— busybox-ash 连通性探针（OK/FAIL），经 ssh 推到路由器/LAN 客户端执行。
...
- `harness_test: 14 passed, 0 failed`
