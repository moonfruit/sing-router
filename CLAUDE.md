...
internal/cli/                # cobra 子命令（status/start/stop/restart/check/shutdown/
                             #   logs/script/daemon/install/uninstall/doctor/update/version）
...
internal/shell/              # bash runner（startup.sh / teardown.sh）
...
- **koolshare**：`/koolshare/init.d/N99sing-router.sh` 监听 `start_nat` → 调 `sing-router restart`
- **merlin**：写 `nat-start` / `services-start` nvram 钩子（用 BEGIN/END marker 增量注入）；nat-start 调 `sing-router restart`

## 关键架构约束（容易踩的坑）

### 重启路径统一：Shutdown / Startup / Restart 三段式
所有"会动 sing-box / iptables"的入口都收敛到 `Supervisor` 的三个原子方法（`internal/daemon/supervisor.go`）：
- **`Shutdown(ctx)`**：拆 iptables（teardown.sh）+ killChild（SIGTERM → grace → SIGKILL）→ state=stopped。幂等。
- **`Startup(ctx)`**：启 sing-box → ready check → 跑 startup.sh 重装 iptables → state=running。失败 → Fatal。
- **`Restart(ctx)` = Shutdown + Startup**，受 **2s 节流闸门**保护（同窗口内重复 Restart 直接 skip）。`RestartForce` 内部入口绕节流，仅 Applier 失败 revert 后用。
...
sha256 真变化闸门保留（"是否触发 Restart"的判断）：gitee 上游会在内容不变时给新 etag，对每个资源都以最终落盘内容的 sha256 与 `var/apply-state.json` 中上次成功 apply 的快照比对，**完全相同则 no-op，不触发 Restart**。
...
`daemon.toml` 与 `config.d/*.json` 的安装语义见 `internal/install/seed.go::writeDefaultAndSeed`：三态——`xxx` 不存在则写 `xxx`、不产生 `.default`（首装让 daemon 直接能起）；`xxx` 已存在且内容与新嵌入一致则什么都不做；只有 `xxx` 已存在且与新嵌入不一致时，保留用户编辑的 `xxx`、把新内容覆盖写到 `xxx.default` 作为 diff/合并基准。模板见 `assets/daemon.toml.tmpl` / `assets/config.d.default/`。关键节：
...
`[seq]` 段控制把 CLEF 事件推送到远程 Seq（sing2seq v1.3.0 `seq.Sink`）；默认关闭，启用需 `enabled=true` + 非空 `url`。任一不满足 daemon 启动时记一条 `seq.disabled` 事件后继续运行。Source 命名约定：`daemon`（自身业务事件）/ `sing-box`（stderr 解析）/ `sing2seq`（sink 自身诊断 `buffer_overflow` / `post_failed` / `shutdown_post_failed`）。环境变量 `SING_ROUTER_SEQ_URL` / `SING_ROUTER_SEQ_API_KEY` 覆盖对应字段（不动 `enabled`）。
...
- 修改 `assets/` 下任何脚本或默认 config 后，先跑 `go test ./assets/`——`embed_test.go` 用静态特征防住"看起来对实际不能跑"的回归（如 dns.json 的 fakeip range、startup.sh 的必填环境变量、merlin snippet 的 BEGIN/END marker、koolshare 脚本调 `sing-router restart`）。
